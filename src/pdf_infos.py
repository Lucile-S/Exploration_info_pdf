...
if __name__ == "__main__":
    ...
    # Initialization  #
    #-----------------#
    # variable, dictionnary and dataframe Initialization
    seq_id = 1
    ## Create a dataframe df to store all results 
    ...
    for pdf_path in pdf_path_list:
        ...
        for AAV_term, AAV_count in AAV_terms.items():
            ...
            # COmbiNE the 2 dicitonnay and add to a list
            AAV_data.append({**METADATA, **AAV_term_dict})

        METADATA['Linked_references'] =  Linked_references
        print(len(AAV_data))
        # Dataframe filling ( add dictonnay as as row to dataframe)
        df = df.append(METADATA, ignore_index=True)
        IDs_table  = IDs_table.append({'DOI': DOI, 'PMCID': pmcid, 'PMID': pmid, 'Publi_ID':Publi_ID,'Title':Title, 'PDF_name': File_name},ignore_index=True)
        #AAV_df = [AAV_df.append(data, ignore_index=True) for data in AAV_data]
